...
def build_bootloader(dst, bootloader, data, version):
    print() or ''
    for project, platform in data:
        ...
        shutil.copy(
            ...
            '{dst}/MidiBoot-{platform_alias}-{version}.bin'.format(
                dst=dst, platform_alias=platform_alias, version=version))
        shutil.copy(
            '{bootloader}/Build/MidiBoot-{platform}.syx'.format(
                bootloader=bootloader, platform=platform),
            '{dst}/MidiBoot-{platform_alias}-{version}.syx'.format(
                dst=dst, platform_alias=platform_alias, version=version))

def build_project(dst, project, platforms, version):
    print() or ''
    for platform in platforms:
        ...
        shutil.copy(
            ...
            '{dst}/{platform_alias}-{version}.bin'.format(
                dst=dst, platform_alias=platform_alias, version=version)
            )
        shutil.copy(
            '{project}/Build/{platform_alias}.syx'.format(
                project=project, platform_alias=platform_alias),
            '{dst}/{platform_alias}-{version}.syx'.format(
                dst=dst, platform_alias=platform_alias, version=version))

def get_version():
    version = [None, None]
    for line in open('Source/device.h'):
        if line.startswith('#define FIRMWARE_VERSION_MAJOR'):
            words = [word for word in line.split() if word.strip()]
            version[0] = words[-1]
        elif line.startswith('#define FIRMWARE_VERSION_MINOR'):
            words = [word for word in line.split() if word.strip()]
            version[1] = words[-1]
    if any(bit is None for bit in version):
...
def get_bootloader_version():
    version = [None, None]
    for line in open('Source/device.h'):
        if line.startswith('#define BOOTLOADER_VERSION'):
            words = [word for word in line.split() if word.strip()]
            return words[-1].strip('"')
    else:
        raise Exception("No bootloader version found")
...
def main(config, smp):
    ...
    print('Bootloader version {version}'.format(version=bootloader_version))
    ...
    for bootloader, items in itertools.groupby(
        ...
        if bootloader is not None:
            if smp:
                ...
                build_bootloader(
                    dst, bootloader, list(item[1:] for item in items),
                    bootloader_version)
    ...
    for project, items in itertools.groupby(
        ...
        if smp:
            ...
            build_project(
                dst, project, list(item[2] for item in items),
                version)
