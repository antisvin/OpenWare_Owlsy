...
public:
  void begin() override {
    ...
    total_sectors = (uint32_t)&_FLASH_STORAGE_SIZE / STORAGE_SECTOR_SIZE;
  ...
  void loop() override {
    if (state < total_sectors){
      ...
      (void)p;
      p = &buf[0];
      int size = state * STORAGE_SECTOR_SIZE / 1024;
      if (size > 999) {
      ...
      p = stpcpy(p, " / ");      
      size = total_sectors * STORAGE_SECTOR_SIZE / 1048576;
      p = stpcpy(p, msg_itoa(size, 10));
      p = stpcpy(p, (const char*)"M");
      debugMessage(buf);
#endif
...
public:
  void begin() override {
    ...
    state = DEFRAG_COPY;
    sector = 0;
    storage_sectors = (uint32_t)&_FLASH_STORAGE_SIZE / STORAGE_SECTOR_SIZE;
    if ((uint32_t)&_FLASH_STORAGE_SIZE % STORAGE_SECTOR_SIZE){
      storage_sectors++;
    }
    block = 0;
    written_sector = 0;
    total_blocks = storage.getBlocksTotal();
      ...
      case DEFRAG_COPY:
        if (block < total_blocks){
          ...
          if (storage_block.verify()){
            ...
            buffer_sectors = (buffer_end - buffer_start) / STORAGE_SECTOR_SIZE;
            if ((buffer_end - buffer_start) % STORAGE_SECTOR_SIZE)
              buffer_sectors++;
          }
          block++;
        }
...
#endif
          storage.write(
            written_sector * STORAGE_SECTOR_SIZE, (void*)buffer_start,
            min(STORAGE_SECTOR_SIZE, buffer_end - buffer_start));
          buffer_start += STORAGE_SECTOR_SIZE;
          written_sector++;
        }
        else {
          state = DEFRAG_DONE;
...
private:
  ...
  uint8_t sector, block, written_sector;
  uint8_t storage_sectors, total_blocks, buffer_sectors;
